...
function uf_SaveFile(const aFileName, aFileData: String; const aUseDateTime: Boolean = True): Boolean;
  function uf_StringCrop(const aStr, aStrPosBegin, aStrPosEnd: String; const aDepth: Integer = 1): String;
  procedure P_onInputTypeSubmit(indexItem: Integer; tagName: string; webBrowser: TWebBrowser);
  procedure P_onInputTagName(ByName: string; value: string; WebBrowser: TWebBrowser);
...
var
  ...
  Ph: DWORD;
begin
  //ShellExecute();//   SchTasks /Create /SC DAILY /TN �My Task� /TR �C:RunMe.bat� /ST 09:00
  Result := true;
...
var
  o: OleVariant;
begin
  o := WebBrowser.oleobject.Document.forms.item[0].all;
  o.tags(tagName).item(indexItem).click();
end;

procedure P_onInputTagName(ByName: string; value: string; WebBrowser: TWebBrowser);
var
  name: IHTMLElementCollection;
  el: IHTMLInputElement;
begin
  name := (WebBrowser.Document as IHTMLDocument3).getElementsByName(ByName);
  if name.length <> 0 then
  begin
    el := name.item(0, 0) as IHTMLInputElement;
    el.value := value;
  end;
end;

function uf_SaveFile(const aFileName, aFileData: String; const aUseDateTime: Boolean): Boolean;
var
  mInFile: TextFile;
  mFile: String;
begin
...
function uf_StringCrop(const aStr, aStrPosBegin, aStrPosEnd: String; const aDepth: Integer): String;
var
