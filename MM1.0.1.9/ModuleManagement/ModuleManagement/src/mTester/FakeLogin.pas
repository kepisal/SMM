...
begin
  ...
  i := 0;
  j := 0;
  //fullFileName:='bin\data.txt';
  //fullFileName:=Application.ExeName;
  fullFileName:=ExtractFilePath(Application.ExeName)+'bin\data.txt';
...
//    uSaveFile('D_data.txt',str);
    ...
    begin
      ...
      if (isName) and (ispwd) then
        // show message
      begin
        FileEncryption(ExtractFilePath(Application.ExeName)+'bin\auto.ini','auto.ini','autorun=[0]_1_');
        ShellExecute(0, 'open', PChar(ExtractFilePath(Application.ExeName)+'bin\MTester.exe'), PChar(''), PChar(''), SW_Hide);
        Self.Close;
      end
      else
