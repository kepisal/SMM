...
uses
  Windows, ActiveX, Variants, SysUtils, StrUtils, Classes, Messages, Forms, ComCtrls, SHDocVw_TLB, MSHTML;
...
type
  ...
  Public
    ...
    function WaitForNavigate(const lpath : String):Boolean;
    ...
    //constructor Create(const dHandle: THandle); // TWebBrower
    constructor Create();
...
function TMyAgent.WaitForNavigate(const lpath : String) : Boolean;
begin
  Result := True;
...
//constructor TMyAgent.Create(const dHandle: THandle);  // TWebBrower
constructor TMyAgent.Create();
begin
  try
    WebBrowser := TInternetExplorer.Create(nil);
    WebBrowser.Connect;
    WebBrowser.Visible := False;
    //WebBrowser.ParentWindow := dHandle;               // TWebBrower
    //WHandle := dHandle;
    {
